...
import com.jee4a.oss.admin.common.config.CacheKeys;
...
import com.jee4a.oss.framework.custom.LoginInfoAnnotation;
	...
	public LoginInfoVO decLoginInfo(String token, HttpServletRequest request) {
		try {
			...
			if (loginInfoVO.getExpireTime().getTime() < System.currentTimeMillis()) {
				...
				// 如果有效期小于最大值的某个系数则续期
				if ((loginInfoVO.getExpireTime().getTime() - System.currentTimeMillis()) < (2 * 60 * 60) * 1000 * 0.3) {
					loginInfoVO.setExpireTime(DateFormatUtils.addSec(new Date(), 2 * 60 * 60));
					try {
